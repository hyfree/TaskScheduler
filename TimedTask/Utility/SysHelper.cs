...
namespace TimedTask.Utility
{
    public class SysHelper
    {
        ...
        {
            string app = Read(TimedTask.Model.PM.ApplicationName).Trim();
        ...
        public void AutoStartup(bool isRun, string exePath)
        {
            try
            {
                if (isRun)
                {
                    registryKeyApp.SetValue(TimedTask.Model.PM.ApplicationName, exePath);
                }
                else
                {
                    registryKeyApp.DeleteValue(TimedTask.Model.PM.ApplicationName, false);
                }
            }
            catch (Exception ex)
            {
